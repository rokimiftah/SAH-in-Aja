import type { BusinessInfo, Ingredient, Product, TemplateType } from "@features/dokumen-halal";
...
const INITIAL_PRODUCTS: Product[] = [{ id: "p1", name: "" }];

const INITIAL_INGREDIENTS: Ingredient[] = [{ name: "", supplier: "", halalStatus: "perlu_verifikasi", productsUsedIn: [] }];
...
export function DokumenHalalPage() {
  ...
  const [blockingModalType, setBlockingModalType] = useState<"not_eligible" | "high_risk" | null>(null);
  ...
  const handleStartCreate = () => {
    ...
    if (!eligibility) {
      setBlockingModalType("not_eligible");
      return;
    }

    if (eligibility.riskLevel === "high") {
      setBlockingModalType("high_risk");
      return;
    }
  ...
  const handleBlockingModalAction = () => {
    if (blockingModalType === "not_eligible") {
      navigate("/dashboard/cek-jalur");
    }
    setBlockingModalType(null);
  };
  ...
  const handleGenerate = async () => {
    ...
    await generateDocument(selectedTemplate, businessInfo, validProducts, validIngredients);
  };
  ...
  const hasValidProducts = products.some((p) => p.name.trim() !== "");
  ...
  return (
    <>
      {blockingModalType && (
        <BlockingModal type={blockingModalType} onClose={() => setBlockingModalType(null)} onAction={handleBlockingModalAction} />
      )}
      <PageContainer
      ...
      >
        ...
        {displayState === "products" && (
          <div>
            <ProductsForm data={products} onChange={setProducts} />
            <div className="mt-6">
              <button
                type="button"
        ...
        {displayState === "ingredients" && (
          <div>
            <IngredientsForm data={ingredients} products={products} onChange={setIngredients} />
            <div className="mt-6">
              <button
                type="button"
